...
/*! \brief A generic in-game text class with newline support. The class uses "\n" as the newline symbol
    ...
    list of varibles (get/set):
        ...
        - type: int
            ...
            - justify_horizontal - where the text is horizontaly in relation to the X/Y coordinates
                - TEXT_JUSTIFY_LEFT
                - TEXT_JUSTIFY_RIGHT
                - TEXT_JUSTIFY_CENTER
            - justify_vertical - where the text is verticaly in relation to the X/Y coordinates
                - TEXT_JUSTIFY_TOP
                - TEXT_JUSTIFY_BOTTOM
                - TEXT_JUSTIFY_MIDDLE
            - lines [read-only] - number of lines the text occupies
            - size [read-only] - number of chars
        - type: float
            - x - x position
            - y - y position
            - z - z position
        - type: string
            ...
            - font - ID string of the font to be used
            - newline [read-only] - char used to for newline ('\n' in most cases)
*/
...
private:
    ...
    //Default behaviors
    const static char newline_symbol='\n';
    const static char newline_symbol_length=1;
    const static bool default_enable=true;
